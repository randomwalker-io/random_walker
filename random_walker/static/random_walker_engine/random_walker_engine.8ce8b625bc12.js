...
// The get current location code is from:
// http://themarklee.com/2013/10/27/super-simple-geolocation/
...
var options = {
    // enableHighAccuracy = should the device take extra time or power to return
    // a really accurate result, or should it give you the quick (but less
    // accurate) answer?
    enableHighAccuracy: true,
    // timeout = how long does the device have, in milliseconds to return a
    // result?
    timeout: 5000,
    // maximumAge = maximum age for a possible previously-cached position. 0 =
    // must return the current position, not a prior cached position
    maximumAge: 0
};
...
function initialise(pos){
    // Setting the width and height of the map element. Although not the best
    // practice it helps to make sure the size is valid
